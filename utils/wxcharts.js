...
function drawColumnDataPoints(series, opts, config, context) {
    ...
    series.forEach(function (eachSeries, seriesIndex) {
        ...
        points.forEach(function (item, index) {
            if (item !== null) {
                if (opts.columnColors) {
                  context.setFillStyle(opts.columnColors[index] || eachSeries.color);
                  context.beginPath();
                }
                ...
                if (opts.columnColors&&opts.columnColors[index]){
                  context.fill();
                }
            }
        });
...
function drawYAxis(series, opts, config, context) {
    ...
    context.stroke();
    context.beginPath();
    context.setFontSize(opts.yAxis.fontSize||config.fontSize);
...
function drawPieDataPoints(series, opts, config, context) {
    ...
    if (opts.dataLabel) {
      ...
      radius -= 2 * (opts.ringPadding||config.padding);
    }
