...
Page({
  ...
  weike_list: function (e) {
    ...
    network.requestLoading('/user/weike_list', params, this.data.weikeList ? "" : "努力加载中...", res => {
      if (res.status == 1) {
  ...
  weike_zan: function (e) {
    ...
    let url = '/user/weike_zan';
    let params = { "weikeId": this.data.weikeList[index].weikeId };
  ...
  weike_collect: function (e) {
    let index = e.currentTarget.dataset.index;
    let url = '/user/weike_collect';
    let params = { "wid": this.data.weikeList[index].weikeId };
  ...
  toDetail: function (e) {
    wx.navigateTo({
      url: '/pages/student/weike/detail/detail?weikeId=' + e.currentTarget.dataset.id,
