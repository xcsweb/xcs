...
Page({
  ...
  onLoad: function (options) {
    this.setData({
      classesHourTeachId: options.classesHourTeachId
    });
    this.findDetail();
  },

  findDetail: function(){
    network.request("/user/classes_hour_teach_detail",
      {
        "classesHourTeachId": this.data.classesHourTeachId
      },
      res => {
        if (res.status == 1) {
