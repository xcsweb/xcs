...
Page({
  ...
  data: {
    student: {},
    tabs: ['基本资料', '费用记录', '上课记录', '考级评测', '积分记录', '跟进记录', '测试记录'],//, '学情报告'
    selectedTabIndex: 0,
    showSecondLineTabs: true,
    ...
    network.requestLoading('/manage/stu_detail', {
      ...
      if (res.status == 1) {
        ...
        if (tab == 1) {
          ...
          res.data.areaCampuses.forEach((areaCampuses, areaCampusesIndex) => {
            areaCampuses.campusVos.forEach((campusVos, campusVosIndex) => {
              if (campusVos.campusId == res.data.student.campusId) {
                console.log("campusVosIndex", campusVosIndex)
                this.setData({
          ...
          //studentCoursesIndex
          res.data.courses_ = [];
          if (res.data.student.campusId && res.data.student.saleCourseId) {
        ...
        if (tab == 2) {
          res.data.forEach(d => {
            d.day_ = util.formatTime(new Date(d.createDt * 1000))
          })
        }
        if (tab == 3) {
          res.data.forEach(d => {
        ...
        if (tab == 4) {
          res.data.forEach(d => {
            d.day = util.formatTime(new Date(d.promotionDt * 1000)).substr(0,10);
          })
        }
        ...
        if (tab == 5) {
          ...
          res.data.changeRecord.forEach(d => {
            d.createDt = util.formatTime(new Date(d.createDt * 1000))
            d.handleDt = util.formatTime(new Date(d.handleDt * 1000))
          })
        }
  ...
  //打电话
  calling: function() {
    if (!this.data.student.tel) {
      return;
    }
    wx.makePhoneCall({
      phoneNumber: this.data.student.tel,
      success: function() {
        console.log("拨打电话成功！")
      },
      fail: function() {
  ...
  changeAvatar:function(){
    util.chooseImgUpload(res => {
      network.requestLoading('/manage/modify_stu_headimg', { "studentId": this.data.student.studentId, "headImg": res.key }, "", res2 => {
        if (res2.status == 1) {
          this.setData({
            ["student.avatar"]: res.imageURL,
            ["tab1Data.student.avatar"]: res.imageURL
          })
        } else {
          util.toast("设置头像失败");
        }
      }, error => {
        util.toast("设置头像失败" + JSON.stringify(error));
      })
    }, error => {
  ...
  tps_big_class_change: function (e) {
    ...
    wx.navigateTo({
      url: '/pages/potential_stu/tps_item_record_batch/tps_item_record_batch?big_class_id=' + big_class_id + "&studentId=" + studentId,
    })
  },
  //删除体测记录
      ...
      if (res.status == 1) {
        ...
        if (data1.length > 0)
          setTimeout(() => {
            ...
            _wxCharts.addEventListener('tap', e => {
              console.log(11111)
              var index = _wxCharts.getCurrentDataIndex(e);
              console.log(index)
            });
          }, 100)
      } else {
  ...
  tab1PickerChangestudentCourses: function(e) {
    let index = Number.parseInt(e.detail.value);
    this.setData({
      ["tab1Data.student.studentCoursesName"]: this.data.tab1Data.studentCourses[index].name,
      ["tab1Data.student.studentCoursesId"]: this.data.tab1Data.studentCourses[index].studentCourseId,
      studentCoursesindex: index
    })
  },
  tab1PickerChangeclassesInfo: function(e) {
    let index = Number.parseInt(e.detail.value);
    this.setData({
      ["tab1Data.student.classesInfoclassesId"]: this.data.tab1Data.classesInfo[index].classesId,
      ["tab1Data.student.classesInfoclassesName"]: this.data.tab1Data.classesInfo[index].classesName,
      studentCoursesindex: index
    })
  },
  ...
  stu_edit_sub: function() {
    let params = {
      ...
      "saleMemberId": 3,
      "saleCourseId": 1,
      "saleCoursePrice": this.data.tab1Data.student.saleCoursePrice,
      "sourceCrmtagId": this.data.tab1Data.student.sourceCrmtagId,
      "user": {
        "userId": 1,
        "userName": "admin",
        "password": ""
      },
      "studentParentList": this.data.tab1Data.student.studentParentList
    };
    ...
    if (!util.isPhone(this.data.tab1Data.student.tel)) {
    ...
    if (this.data.saleCourseIndex) {
      params.saleCourseId = this.data.tab1Data.course_[this.data.saleCourseIndex].courseId
    }
  ...
  bindinputjz: function(e) {
    ...
    console.log(this.data.tab1Data.student.studentParentList)
  },
  //发通知
  notice_add_sub: function() {
    let params = {
      ...
      "content": "",
      "noticeType": "2",
      "sendDt": new Date().getTime() / 1000,
      "noticeRecords": [{
        "studentId": this.data.tab1Data.student.studentId,
        "userId": this.data.tab1Data.student.userId
      }, ]
    };
    if (!this.data.title) {
      wx.showToast({
  ...
  onShow: function () {
    wx.getStorage({
      ...
      success: res => {
        ...
        this.stu_detail();
        if (this.data.selectedTabIndex == 6 && this.data.tab7BtnIndex == 1) {
          this.get_stu_tps_curve_list();
        }
      },
    });
