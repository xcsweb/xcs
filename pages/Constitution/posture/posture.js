...
Page({
    ...
    network.request("/manage/get_tps_item_category", {
      ...
      res => {
        ...
        for (let i = 0; i < q.tpsItemList.length; i++) {
         ...
         let info=res.data
         let p_id=this.data.itemid
         let infos=[]
        for (let i = 0; i < q.tpsItemList.length; i++){
          if (p_id == q.tpsItemList[i].id){
            infos.push(q.tpsItemList[i])
           }
         }
        this.setData({
          infos: infos,
        ...
        for (let i = 0; i < q.tpsItemList.length; i++) {
          if (p_id == q.tpsItemList[i].id) {
            this.setData({
              content: q.tpsItemList[i].content
            })
          }
        }
        ...
        let textdata = []
        for (let i = 0; i < infos[0].tpsCategoriesOption.length; i++){
            textdata.push({
              id:i,
              name: infos[0].tpsCategoriesOption[i].title,
              optionId: infos[0].tpsCategoriesOption[i].id
            })
        }
  ...
  onChangeShowState: function (e) {
    ...
    // console.log(index)
    // console.log(value)
    // console.log(this.data.textdata[value].name)
    this.setData({
      val1: this.data.textdata[value].name,
      optionId: this.data.textdata[value].optionId,
    })
    // console.log(this.data.studentname[index])
    // console.log(this.data.studentId[index].studentId)
    ...
    let params = {
      ...
      "studentId": this.data.studentId[index].studentId,
    }
    this.setData({
      [x]: params,
    })
