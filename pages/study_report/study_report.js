...
Page({
  ...
  getFilter: function () {
    network.request(
      "/manage/filter_option", { campus: {"relation":2}, classes: 1 },
      res => {
        if (res.status == "1") {
          this.setData({
  ...
  get_study_report_list: function() {
    ...
    this.data.searchParam.reportNumOverZero = 1;
    network.request('/manage/get_study_report_list', this.data.searchParam, 
      res => {
        if (res.status == 1) {
          res.data.forEach(e => {
